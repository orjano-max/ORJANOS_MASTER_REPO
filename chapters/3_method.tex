...
\begin{itemize}
    \item A mobile robotic platform capable of accomodating a robotic manipulator.
...
%Aj 3.3 can be renamed Hardware architecture. remove subsection 3.3.1,3.3.2
...
\end{figure}
%aj only of figure for a subsection. doesn't look good
...
%Aj Not redeable. Make it bigger and font also larger.
%aj only of figure for a subsection. doesn't look good
\begin{figure}[H]
  \centering
...
The UGV needs some form of perception system to help determine its absolute position within the environment. This is done through the perception system described in section \ref{sec:M:AN:Perception}.
Lastly has to be a map of the surrounding environment, so that the UGV can place itself somewhere on this map. This map is created with the use of SLAM \ref{sec:M:AN:LocalisationAndMapping}.
With all these requirements fulfilled, an autonomous navigation system can be put together. The navigation system will take care of both global and local path planning, and send velocity commands to the motion control system based on the calculations. A high level illustration of how the different subsystems interact to achieve autonomous navigation, can be seen in figure \ref{fig:M:AN:ANMethod}.
...
An adaptation of Clearpath's own Husky A200 ROS2 packages are used to operate the UGV in ROS2. 
...
The localisation and mapping system combines several subsystems that work together to achieve SLAM. The SLAM algorithm itself is provided by the ROS2 package SLAM Toolbox. SLAM Toolbox utilises a pose-graph based approach to the SLAM problem.
